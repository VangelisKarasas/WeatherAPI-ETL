...
def get_weather_data(locations: dict, api_key: str) -> dict:
    ...
    for location_name, location in locations.items():
        try:
            ...
            weather_data["current"][location_name] = fetch_api_data(
                current_url)
            weather_data["forecast"][location_name] = fetch_api_data(
                forecast_url)
...
def transform_current_weather_data(data_dict: dict) -> pd.DataFrame:
    ...
    # Check if 'weather' is a list and select the first element
...
def transform_forecasted_weather_data(data_dict: dict) -> pd.DataFrame:
    ...
    for forecast_item in forecasts_dict:
        ...
        forecast_df = pd.concat(
            [forecast_df, forecast_item_df], ignore_index=True)
...
def upload_json_to_gcs(json_data: dict, bucket_name: str, folder_path: str) -> None:
    ...
    # Using os.path.join for cleaner path handling
    object_path = os.path.join(folder_path, filename)

    # Convert JSON to bytes and upload
    blob = bucket.blob(object_path)
    blob.upload_from_string(json.dumps(json_data).encode(
...
def main(request: dict) -> str:
    ...
    for location, data in weather_data['current'].items():
        ...
        current_weather = pd.concat(
            [current_weather, transformed_data], ignore_index=True)
    ...
    for location, data in weather_data['forecast'].items():
        ...
        forecast_weather = pd.concat(
            [forecast_weather, transformed_data], ignore_index=True)

    # Define bucket name for storing data
    bucket_name = "raw_weather_api_data_new_v4"
    ...
    upload_df_to_bigquery(dataframe=current_weather, project_id=settings.project_id,
                          dataset_id=settings.dataset_id, table_name='current_weather')
    upload_df_to_bigquery(dataframe=forecast_weather, project_id=settings.project_id,
                          dataset_id=settings.dataset_id, table_name='forecasted_weather')
